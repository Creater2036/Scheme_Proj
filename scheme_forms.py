...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            if not isinstance(clause.first, Pair) and clause.first != 'else':
                return eval_all(clause, env)
            if clause.rest is nil:
                if clause.first.first == 'quote':
                    return clause.first.rest.first
                return is_scheme_true(1)
